...
import com.si7ateck.dz.MainActivity
import com.si7ateck.dz.R
import com.si7ateck.dz.ui.doctor.DoctorItemFragment
import com.si7ateck.dz.utility.customview.NoScrollRecyclerView
...
class FiltersLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0)
    : FrameLayout(context, attrs, defStyleAttr) {
     ...
     var MyContext : DoctorItemFragment? = null
    ...
    private lateinit var fabFilterIcon: ImageView
    private lateinit var fabCloseIcon: ImageView
    private lateinit var mainContainer: View
    private lateinit var filtersContainer: View
    private lateinit var tabsContainer: View
    private lateinit var tabsRecyclerView: NoScrollRecyclerView
    private lateinit var viewPager: ViewPager2
    private lateinit var bottomBarCardView: CardView
    ...
    private val bottomBarPinkColor: Int by bindColor(R.color.primaryLightColor)
    private val bgColor: Int by bindColor(R.color.primaryDarkColor)
    ...
    init {
        ...
        fabFilterIcon= findViewById(R.id.fab_filter_icon)
        fabCloseIcon= findViewById(R.id.fab_close_icon)
        ...
        bottomBarCardView= findViewById(R.id.bottom_bar_card_view)
        ...
        fab.setOnClickListener {
            ...
            Log.d("akramsearchTest","working fine")

            if ((context as MainActivity)!= null){
                Log.d("akramsearchTest","yoyoyoyoyoyoyoyo")
            }


            if (MyContext != null ) {
                if ((MyContext as DoctorItemFragment) != null) {
                    Log.d("akramsearchTest", "nooooooooo")
                }
            }else{

                Log.d("akramsearchTest", "MyContext is null")

            }

            if ((MyContext as DoctorItemFragment).isAdapterFiltered) {
                unFilterAdapterWithFabAnimation()
                Log.d("akramsearchTest","isAdapterFiltered true ")

            } else {
                // We're opening the fab so set the adapters and open filter sheet
                tabsHandler.setAdapters(true)
                openFilterSheet(true)
                Log.d("akramsearchTest","isAdapterFiltered false ")
    ...
    private fun openFilterSheet(isOpening: Boolean) {
        ...
        val scaleDownAnimator = (MyContext as DoctorItemFragment).getAdapterScaleDownAnimator(isOpening)
    ...
    private fun onFilterApplied() {
        ...
        // Close icon rotates to simulate loading while adapter items in DoctorItemFragment are filtered
        ...
        closeIconLoadingAnimator.doOnStart {
            ...
            (MyContext as DoctorItemFragment).isAdapterFiltered = true
        }
        ...
        val scaleDownAnimator = (MyContext as DoctorItemFragment).getAdapterScaleDownAnimator(false)
        scaleDownAnimator.duration = pathAnimator.duration

        // AnimatorSet choreography of the animation: (1,2,3) -> 4 -> (5,6)
        // At the end of the animation, DoctorItemFragment shows it's fab. removes this fragment and we
    ...
    private fun unFilterAdapterWithFabAnimation() {
        ...
        val scaleDownAnimator = (MyContext as DoctorItemFragment).getAdapterScaleDownAnimator(true)
        ...
        closeIconLoadingAnimator.doOnStart { (MyContext as DoctorItemFragment).isAdapterFiltered = false }
        ...
        val scaleUpAnimator = (MyContext as DoctorItemFragment).getAdapterScaleDownAnimator(false)
     ...
     * of animation, we ask MainListAdapter from DoctorItemFragment to start it's animations as well.
     */
    @RequiresApi(Build.VERSION_CODES.LOLLIPOP)
